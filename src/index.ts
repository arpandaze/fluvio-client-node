...
export interface FluvioClient {
    ...
    topicProducerWithConfig(topic: string, config: TopicProducerConfig): Promise<TopicProducer>
    partitionConsumer(
        topic: string,
        partition: number
...
export default class Fluvio implements FluvioClient {
     ...
     */
    async topicProducerWithConfig(topic: string, config: TopicProducerConfig): Promise<TopicProducer> {
        this.checkConnection()
